...
function parsing(body) {
  ...
  heads.forEach((head, index) => {
   ...
   headers.value.push( {
    ...
    headers.value[5].sortable=true; 
    headers.value[7].sortable=true; 
    headers.value[0].width=100; 
    headers.value[1].width=250; 
    headers.value[6].width=250;  
    headers.value[7].width=150;  
...
<template>
  ...
  <h2 class="p-3 font-bold"> {{ prepublie ? "Postes Pré-publiés" : "Postes Publiés" }} </h2>
  <div v-if="data" class=" "> 

    <v-text-field class="w-2/6 pb-2"
        v-model="searchValue"
...
.customize-table {
  ...
  --easy-table-body-row-background-color: #c9deff;
  --easy-table-body-row-height: 50px;
  --easy-table-body-row-font-size: 13px;

  --easy-table-body-row-hover-font-color: #2d3a4f;
  --easy-table-body-row-hover-background-color: rgb(219, 234, 254);
